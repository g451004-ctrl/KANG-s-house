...
export default function WeekProgressCard({ task, weekDate, forCheckin, verifiedCount, mode = 'view', onToggle, onCancelVerify }) {
    ...
    <div className={`task-card ${achieved ? 'task-card--done' : ''}`}>
      ...
      <div className="task-card__days">
        {days.map(({ dateStr, label }, dayIndex) => {
          ...
          const childClickable = mode === 'child' && !future && state !== 'verified'
          const parentCancelable = mode === 'view' && onCancelVerify && state === 'verified'
          const clickable = childClickable || parentCancelable
          const handleClick = () => {
            if (childClickable) onToggle(task.id, dateStr)
            else if (parentCancelable) {
              if (window.confirm('승인을 취소할까요? 취소하면 인증 대기 상태로 돌아가요.')) {
          ...
          return (
            <button
              ...
              disabled={!clickable}
              onClick={handleClick}
              className={`day-cell day-cell--${state} ${dateStr === todayStr ? 'day-cell--today' : ''} ${future ? 'day-cell--future' : ''} ${parentCancelable ? 'day-cell--cancelable' : ''}`}
              title={parentCancelable ? '클릭해서 승인 취소' : dateStr}
            >
              <span className="day-cell__label">{label}</span>
